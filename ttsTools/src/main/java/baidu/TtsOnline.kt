...
object TtsOnline {
    ...
    fun dotts(ttsInput: String, token: String, fileName: String = "") {
        val encodeTtsInput = ttsInput.urlEncode().urlEncode()
        val bodyText =
            "tex=$encodeTtsInput&lan=zh&ctp=1&per=$per&spd=$spd&pit=$pit&vol=$vol&cuid=$cuid&tok=$token&aue=$aue"
        ...
        if (response.code() != 200) {
            ...
            if (responseBody.contentType()?.type() == "audio") {
                ...
                val file = File(
                    pd
                        ?: "", "${if (fileName.isEmpty()) ttsInput else fileName}.mp3"
                )
                val buffer = file.sink().buffer()
                buffer.writeAll(responseBody.byteStream().source())
                buffer.flush()
                Log.d("tts", "dotts success : ${file.absolutePath}")
            } else {
